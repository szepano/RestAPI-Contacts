...
class Auth:
    ...
    async def create_access_token(self, data: dict, expires_delta: Optional[float] = None):
        ...
        encoded_access_token = jwt.encode(to_encode, self.SECRET_KEY, self.ALGHORITM)
    ...
    async def create_refresh_token(self, data: dict, expires_delta: Optional[float] = None):
        ...
        encoded_refresh_token = jwt.encode(to_encode, self.SECRET_KEY, self.ALGHORITM)
        return encoded_refresh_token

    async def decode_refresh_token(self, refresh_token: str):
        try:
            payload = jwt.decode(refresh_token, self.SECRET_KEY, self.ALGHORITM)
    ...
    async def get_current_user(self, token: str = Depends(ouath2_scheme), db: Session = Depends(get_db)):
